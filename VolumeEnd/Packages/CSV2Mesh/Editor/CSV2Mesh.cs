...
namespace CSV2Mesh
{
    public class CSV2Mesh : EditorWindow
    {
        ...
        static int data_uv_id1 = 22;
        static int data_uv_id2 = 26;
        static int data_uv_id3 = 30;
        ...
        private void OnGUI()
        {
            ...
            if (dataTypes)
            {
                ...
                data_uv_id1 = EditorGUILayout.Popup("UV1: ", data_uv_id1, Head.ToArray(), EditorStyles.popup); 
                data_uv_id2 = EditorGUILayout.Popup("UV2: ", data_uv_id2, Head.ToArray(), EditorStyles.popup); 
                data_uv_id3 = EditorGUILayout.Popup("UV3: ", data_uv_id3, Head.ToArray(), EditorStyles.popup); 
        ...
        public static int ImportMeshFromRenderDoc(string assetPath, bool needRotationN90, bool reverseUvY, bool reverseTris, List<string> head)
        {
            ...
            var uv1_x = GetColumnIndex(heads, head[data_uv_id1] + ".x");
            var uv1_y = GetColumnIndex(heads, head[data_uv_id1] + ".y");
            var uv2_x = GetColumnIndex(heads, head[data_uv_id2] + ".x");
            var uv2_y = GetColumnIndex(heads, head[data_uv_id2] + ".y");
            var uv3_x = GetColumnIndex(heads, head[data_uv_id3] + ".x");
            var uv3_y = GetColumnIndex(heads, head[data_uv_id3] + ".y");
            ...
            if (IDX < 0 || position_x < 0 || position_y < 0 || position_z < 0
                || uv_x < 0 || uv_y < 0 || uv1_x < 0 || uv1_y < 0 || uv2_x < 0 || uv2_y < 0 || uv3_x < 0 || uv3_y < 0 )
            {
            ...
            Vector2[] outputUv1s = new Vector2[vertexLength];
            Vector2[] outputUv2s = new Vector2[vertexLength];
            Vector2[] outputUv3s = new Vector2[vertexLength];
            ...
            for (int i = 0; i < allRows.Count; ++i)
            {
                ...
                if (realIndex < outputVertexs.Length && realIndex >= 0)
                {
                    ...
                    outputUv1s[realIndex] = new Vector2(currLine[uv1_x], reverseUvY ? 1 - currLine[uv1_y] : currLine[uv1_y]);
                    outputUv2s[realIndex] = new Vector2(currLine[uv2_x], reverseUvY ? 1 - currLine[uv2_y] : currLine[uv2_y]);
                    outputUv3s[realIndex] = new Vector2(currLine[uv3_x], reverseUvY ? 1 - currLine[uv3_y] : currLine[uv3_y]);
                }
                else
                {
                    return -5;
